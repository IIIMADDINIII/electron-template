...
export type CacheId = `${ApiId}-${ApiOrigin | undefined}`;
export type CacheMap = Map<CacheId, { api: unknown, removeListener: () => void; }>;
...
export interface InternalWindowApiPrefab<T> {
  ...
  getApi(id: ApiId, origin: ApiOrigin | undefined, timeout: number, clearCache: () => void): Promise<ApiPrefab<T>>;
}
